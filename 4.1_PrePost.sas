...
data data1;
	set data1;
	by gvkey fyear;
	if dvc>0 then divind=1; else divind=0;
...
proc sql;
	...
	as select a.*, b.group1, b.group2, b.group3, b.group4, b.indyear from data1 as a
	inner join indicators as b
	on a.gvkey=b.gvkey
	and b.indicator=-1
	and a.indicator<>1
	and a.fyear<>b.indyear;
quit;
...
quit;
	...
	/*Winsorization*/
%winsorize(inset=gopublic2, outset=gopublic3, sortvar=fyear, 
vars= 	salgrowth lev da opprofit profit div1 div2 payout1 payout2 repurch1 repurch2,
perc1=1, trim=0);

	/*Export to Stata*/
proc export data=gopublic3
	outfile= "/home/kennesaw/Sunay/Dividends/gopublic.dta" replace;
run;
...
run;
	...
	/*Winsorization*/
%winsorize(inset=goprivate2, outset=goprivate3, sortvar=fyear, 
vars= 	salgrowth lev da opprofit profit div1 div2 payout1 payout2 repurch1 repurch2,
perc1=1, trim=0);

	/*Export to Stata*/
proc export data=goprivate3
	outfile= "/home/kennesaw/Sunay/Dividends/goprivate.dta" replace;
run;
